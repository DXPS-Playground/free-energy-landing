...
export default function LeadForm() {
  ...
  const [d, setD] = useState({
    businessName:"",contactName:"",phone:"",email:"",address:"",bestTime:"",
  ...
  function set(k,v){ setD(p=>({...p,[k]:v})); setErrs(p=>({...p,[k]:null})); }
  ...
  function validate(){
    const e={};
    REQ[step].forEach(f=>{ if(!d[f]||!String(d[f]).trim()) e[f]="Required"; });
    return e;
  }
  ...
  async function submit(){
    ...
    //
    // BEFORE (insecure — API key exposed in browser):
    //   Form → calls api.anthropic.com directly → needs API key in frontend
    //
    // AFTER (secure — API key stays on server):
    //   Form → calls YOUR server at /api/qualify → server adds API key →
    //   server calls api.anthropic.com → server sends to Make webhook →
    //   server returns AI result to form
    //
    // The form just sends the raw lead data as JSON. The server handles
    // everything else: building the prompt, calling Claude, forwarding
    // to Make, and returning the result.
    // ─────────────────────────────────────────────────────────────────────
    ...
    try {
      const res = await fetch("/api/qualify", {
        ...
        body: JSON.stringify({
          // Send all the form fields to the server
      ...
      // The server returns the AI result directly as JSON
      // (score, tier, summary, savings estimate, outreach email, etc.)
  ...
  // ── small components ──
  function F({name,label,req,children}){
    return(
      <div className="field">
  ...
  function In({name,...p}){
    return <input className={errs[name]?"err":""} value={d[name]} onChange={e=>set(name,e.target.value)} {...p}/>;
  }
  function Sel({name,opts,ph}){
    return(
      <select className={errs[name]?"err":""} value={d[name]} onChange={e=>set(name,e.target.value)}>
  ...
  function Checks({name,opts}){
    return(
      <div className="cgrid">
        {opts.map(o=>{
          const on=d[name].includes(o);
          return(
            <div key={o} className={`citem${on?" on":""}`} onClick={()=>set(name,toggle(d[name],o))}>
              <div className="cbox">{on?"✓":""}</div>{o}
            </div>
          );
        })}
      </div>
  ...
  const STEP_CONTENT = [
    ...
    <div className="fgroup">
      <div className="row">
        <F name="businessName" label="Business Name" req><In name="businessName" placeholder="ABC Warehousing LLC"/></F>
        <F name="contactName" label="Contact Name" req><In name="contactName" placeholder="Jane Smith"/></F>
      </div>
      <div className="row">
        <F name="phone" label="Phone" req><In name="phone" type="tel" placeholder="(813) 555-0100"/></F>
        <F name="email" label="Work Email" req><In name="email" type="email" placeholder="jane@company.com"/></F>
      </div>
      <F name="address" label="Property Address" req><In name="address" placeholder="123 Industrial Blvd, Tampa, FL"/></F>
      <F name="bestTime" label="Best Time to Contact">
        <select value={d.bestTime} onChange={e=>set("bestTime",e.target.value)}>
          <option value="">Any time</option>
          {["Mornings (8am–12pm)","Afternoons (12pm–5pm)","Email preferred"].map(o=><option key={o}>{o}</option>)}
        </select>
      </F>
    </div>,
    ...
    // 2 property
    <div className="fgroup">
      <F name="propType" label="Property Type" req><Sel name="propType" opts={PROP_TYPES} ph="Select property type..."/></F>
      <div className="row">
        <F name="sqft" label="Approx. Square Footage" req><In name="sqft" placeholder="e.g. 45,000"/></F>
        <F name="utility" label="Utility Provider" req><Sel name="utility" opts={UTILITIES} ph="Select utility..."/></F>
      </div>
      <F name="role" label="Your Role / Decision Authority" req><Sel name="role" opts={ROLES} ph="Select your role..."/></F>
    </div>,

    // 3 lighting
    <div className="fgroup">
      <F name="ltTypes" label="Current Lighting Type(s)"><Checks name="ltTypes" opts={LT_TYPES}/></F>
      <div className="row">
        <F name="fixtures" label="Approx. # of Fixtures" req><In name="fixtures" placeholder="e.g. 120"/></F>
        <F name="hoursDay" label="Hours Lights On Per Day" req><In name="hoursDay" placeholder="e.g. 16"/></F>
      </div>
      <F name="areas" label="Areas Needing Upgrades"><Checks name="areas" opts={AREAS}/></F>
      <F name="pains" label="What's Driving This Project?"><Checks name="pains" opts={PAINS}/></F>
    </div>,

    // 4 intent
    <div className="fgroup">
      <F name="timeline" label="Project Timeline" req>
        <div className="cgrid">
          {TIMELINES.map(t=>(
            <div key={t} className={`citem${d.timeline===t?" on":""}`} onClick={()=>set("timeline",t)}>
              <div className="cbox">{d.timeline===t?"✓":""}</div>{t}
            </div>
          ))}
        </div>
      </F>
      <F name="notes" label="Anything Else We Should Know?">
        <textarea value={d.notes} onChange={e=>set("notes",e.target.value)} placeholder="Multiple properties, past quotes, specific concerns..."/>
      </F>
      <div className="field">
        <label>Optional Uploads <span style={{color:"var(--muted)",fontWeight:400,textTransform:"none",letterSpacing:0}}>(electric bill, fixture photos, floor plan)</span></label>
        <label className="file-zone">
          <input type="file" multiple accept="image/*,.pdf" onChange={e=>set("fileNames",Array.from(e.target.files).map(f=>f.name))}/>
          📎 Click to attach files
          {d.fileNames.length>0&&<div className="file-names">{d.fileNames.join(", ")}</div>}
        </label>
      </div>
      <div className="promise-box">
        <strong>What happens next:</strong> We'll estimate your energy savings, available utility rebates, and payback period — within 24–48 hours. No cost, no obligation.
      </div>
    </div>,
  ];
    ...
    <>
      ...
      <div className="shell">
        ...
        <div className="panel-left">
          ...
          <div className="panel-title">Request a Free<br/><span className="acc">Lighting Savings</span><br/>Audit</div>
          <div className="panel-sub">Submit your property details and we'll estimate your savings, rebate eligibility, and payback period — at no cost or obligation.</div>
          <div className="benefits">
            {[["💡","Custom savings analysis based on your property, fixture count & utility"],["💰","We identify every available Duke Energy, TECO, or FPL rebate"],["📊","Clear ROI & payback period before you spend anything"],["🤝","We act as your advisor — not just a contractor"]].map(([ic,tx])=>(
              <div className="benefit" key={tx}><div className="bicon">{ic}</div><span>{tx}</span></div>
            ))}
          </div>
          <div className="util-section">
            <div className="util-label">Rebate programs we work with</div>
            <div className="util-chips">{["Duke Energy FL","Tampa Electric","FPL"].map(u=><span className="util-chip" key={u}>{u}</span>)}</div>
          </div>
        </div>
          ...
          {!done ? (
            <>
              <div className="progress-bar">
                {STEPS.map((s,i)=><div key={i} className={`pseg${i<step?" done":i===step?" active":""}`}/>)}
              </div>
            ...
            <div className="loading-wrap">
              ...
              <div className="load-sub">Our AI is reviewing your lighting situation, estimating savings potential, and identifying available utility rebates.</div>
            </div>
          ) : result?.err ? (
            <div className="loading-wrap">
              <div style={{fontSize:"2.5rem"}}>✅</div>
              <div className="load-title">Audit request received!</div>
              <div className="load-sub">We'll review your information and reach out within 24–48 hours with your personalized savings estimate.</div>
            </div>
          ) : result ? (
            <div>
              ...
              <div className="result-card">
                <div className="r-header">
                  ...
                  <div>
                    ...
                    <div className="tier-name">
                      {result.tier==="A"?"High Priority — Contact Today":result.tier==="B"?"Strong Lead — Follow Up Soon":"Nurture Lead — Add to Sequence"}
                    </div>
                  </div>
                </div>
                <div className="r-body">
                  <div>
                    ...
                    <div style={{fontSize:"0.76rem",color:"var(--muted)",marginTop:"0.35rem"}}>{result.tierReason}</div>
                  </div>
                  <div><div className="r-sect-title">Summary</div><div className="r-text">{result.summary}</div></div>
                  <div><div className="r-sect-title">Estimated Savings Opportunity</div><div className="r-text">{result.estimatedSavings}</div></div>
                  <div><div className="r-sect-title">Rebate Eligibility</div><div className="r-text">{result.rebateNote}</div></div>
                  <div>
                  ...
                  <div className="success-note">✓ Lead logged to CRM · AI outreach email ready to send · Slack alert dispatched for Tier {result.tier} leads</div>
                </div>
              </div>
            </div>
          ) : null}
